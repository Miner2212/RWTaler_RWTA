...
void HexAddressConverter::addressChanged(const QString& address) {
    ...
    if(!address.isEmpty()) {
        ...
        hasRWTalerAddress = IsValidDestination(addr);
        if (hasRWTalerAddress) {
            ui->label->setText("Address (RWTaler)");
        ...
        isAddressValid = hasHexAddress || hasRWTalerAddress;
    }
