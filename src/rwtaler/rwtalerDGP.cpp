#include "rwtalerDGP.h"

void RWTalerDGP::initDataEIP158(){
...
bool RWTalerDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule RWTalerDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t RWTalerDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t RWTalerDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t RWTalerDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t RWTalerDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool RWTalerDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void RWTalerDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void RWTalerDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void RWTalerDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void RWTalerDGP::createParamsInstance(){
...
dev::Address RWTalerDGP::getAddressForBlock(unsigned int blockHeight){
...
void RWTalerDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void RWTalerDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void RWTalerDGP::parseStorageOneUint64(uint64_t& value){
...
void RWTalerDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule RWTalerDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
