...
RWTalerState::RWTalerState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = RWTalerState::openDB(_path + "/rwtalerDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

RWTalerState::RWTalerState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute RWTalerState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, RWTalerTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createRWTalerAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            rwtaler::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> RWTalerState::vins() const // temp
{
...
void RWTalerState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* RWTalerState::vin(dev::Address const& _a) const
{
    return const_cast<RWTalerState*>(this)->vin(_a);
}

Vin* RWTalerState::vin(dev::Address const& _addr)
{
...
// void RWTalerState::commit(CommitBehaviour _commitBehaviour)
// {
...
void RWTalerState::kill(dev::Address _addr)
{
...
void RWTalerState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
const dev::Address RWTalerState::createRWTalerAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void RWTalerState::deleteAccounts(std::set<dev::Address>& addrs){
...
void RWTalerState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void RWTalerState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
